...
PURPOSE: Use this file as the controlling prompt for generating TSM 098 and TSM 099 unit tests, diagnostics, practice finals, official finals, answer keys, and compliance reports. It defines content, difficulty, variation, ordering, validation, and institutional-assessment requirements. Do not summarize or reinterpret these requirements before generating an assessment. Apply them directly.
...
- **Part I - Universal generation standards.** Rules that apply to every assessment and every batch.
...
Upload this document and state the assessment name, number of versions, and desired delivery format. Examples include:
...
- "Create Versions A-D of the TSM 098 Fractions Test, with student copies followed by a consolidated answer key."
...
# Part I. Universal generation standards
...
- A new version must use genuinely new numerical values and expressions; merely reordering the same five problems does not create a new version.
...
- Label batch versions clearly as Version A, Version B, and so on, or Version 01, Version 02, and so on for large batches.
...
- Work every problem completely before presenting it. Verify operations, factoring, domains, intersections, discriminants, candidate solutions, and final reductions.
...
## 4. Student-facing presentation

- Begin with the exact assessment title, version label, and short directions applicable to that test.

- Do not label questions by hidden role, such as basic problem, exceptional case, formula problem, or cancellation problem.

- Use conventional mathematical notation, clear grouping symbols, and unambiguous subtraction signs.

- Provide adequate workspace. For graphing questions, provide accurate coordinate grids with the required scale.

- Keep answer keys and compliance notes separate from the student version.

## 5. Standard output package

- **Student copy.** Only the title, directions, questions, tables or grids, and workspace.

- **Instructor answer key.** Essential steps and final answers, including checks and rejected solutions where applicable.

- **Compliance report.** A compact audit showing how the generated version satisfies each required question type and variable constraint.

## 6. Default behavior for future AI requests
...
- If asked for multiple copies or versions, apply every batch rule and produce a separate student copy for each version.

- If the requested delivery format is stated, follow it. If not stated, default to a clean printable format with student versions first, followed by answer keys and compliance reports.
...
**Generation rule:** Each assessment section begins with its five-question map. The detailed requirements that follow are mandatory construction and verification rules, not suggestions.

# 11. TSM 098 Arithmetic Test
...
**Output:** Apply the universal three-part output package: student version, instructor answer key, and compliance report. The key and compliance report must address every test-specific verification item listed above.

# 12. TSM 098 Polynomials Test
...
# 14. TSM 098 Linear Equations and Linear Systems Test
...
# 17. TSM 099 Rational Expressions Test
...
# 18. TSM 099 Quadratic and Rational Equations Test
...
## 20. Exam families and shared architecture
...
## 22. Institutional-item generation protocol
...
The instructor answer key must identify the institutional ID and bank archetype used, show the essential solution, and state a binary correct-answer criterion suitable for institutional reporting. Student copies must not display institutional IDs or archetype labels.

## 23. TSM 098 institutional-assessment bank
...
## 24. TSM 099 institutional-assessment bank
...
## 25. Four supplemental questions - general selection algorithm
...
## 26. TSM 098 supplemental coverage rules
...
## 27. TSM 099 supplemental coverage rules
...
## 28. Institutional reporting controls
...
- The compliance report must list each institutional ID, archetype letter, actual item number, SLO, final answer, and correct/incorrect scoring criterion.
...
Before returning tests created from this guide, the generating system must confirm all of the following.
...
- Student copies reveal no hidden roles, intended methods, exceptional locations, or compliance metadata.

- Answer keys include the required essential steps, checks, rejected candidates, graph features, and cautious conclusions.

- Compliance reports identify every randomized option and certify the version-specific constraints.

- Formatting is readable, notation is unambiguous, graphs are exact, and no answer key appears on a student page.
